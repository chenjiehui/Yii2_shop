...
class CommonController extends Controller
{
    public function init()
    {
        ...
        $cart = Cart::find()->where(['userid' => $userid])->all();
        $this->view->params['cart'] = $cart;
//        $this->view->params['cart'] = $data;
        $tui = Product::find()->where('istui = "1" and ison = "1"')->orderby('createtime desc')->limit(4)->all();
        $new = Product::find()->where('ison = "1"')->orderby('createtime desc')->limit(4)->all();
        $hot = Product::find()->where('ison = "1" and ishot = "1"')->orderby('createtime desc')->limit(4)->all();
        $sale = Product::find()->where('ison = "1" and issale = "1"')->orderby('createtime desc')->limit(4)->all();
